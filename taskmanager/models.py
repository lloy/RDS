...
class FreshInstanceTasks(FreshInstance, NotifyMixin, ConfigurationMixin):
    ...
    def _create_server_volume(self, flavor, image_id, security_groups,
        ...
        LOG.debug('volume_support is %s' % self.volume_support)
    ...
    def _create_server_volume_individually(self, flavor, image_id,
        ...
        def _format_bdm(bdm):
            ...
            device_name = bdm.keys()[0]
            # <id>:[<type>]:[<size(GB)>]:[<delete_on_terminate>]
            mapping = bdm.get(device_name).split(':')
            dmap = {'id': mapping[0],
                    'type': mapping[1],
                    'size': mapping[2],
                    'delete_on_termination': mapping[3]}
            return device_name, dmap
        ...
        blk_1 = {"boot_index": "0",
                 ...
                 "destination_type": "volume",
                 "delete_on_termination": False}
        block_device_mapping_v2.append(blk_1)
        device_name, mapping = _format_bdm(volume_info.get('block_device'))
        blk_2 = {"boot_index": "-1",
                 "device_name": device_name,
                 "uuid": mapping.get('id'),
                 "source_type": "volume",
                 "volume_size": mapping.get('size'),
                 "destination_type": "volume",
                 "delete_on_termination": False}
        block_device_mapping_v2.append(blk_2)
        try:
    ...
    def _prepare_userdata(self, datastore_manager, name):
        ...
        LOG.debug('Cloudinit Path: %s' % cloudinit)
